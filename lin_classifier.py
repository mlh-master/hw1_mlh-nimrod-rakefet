...
def pred_log(logreg, X_train, y_train, X_test, flag=False):
    ...
    w_log = logreg.coef_
    if flag == 'True':
        he = logreg.predict_proba(X_test)
        return y_pred_log, w_log, he
    # -------------------------------------------------------------------------
...
def cv_kfold(X, y, C, penalty, K, mode):
    ...
    for c in C:
        ...
        # ------------------ IMPLEMENT YOUR CODE HERE:-----------------------------
                y_train = y[train_idx]
                y_pred_log, w_log, he = pred_log(logreg, nsd(x_train, mode=mode, flag='False'), y_train, nsd(x_val, mode=mode, flag='False'), flag='True')
                loss_val_vec[k] = log_loss(y[val_idx], he)
                k = k+1
...
def odds_ratio(w, X, selected_feat='LB'):
    ...
    """
    # ------------------ IMPLEMENT YOUR CODE HERE:-----------------------------
    index_of_selected_feat = X.columns.get_loc(selected_feat)

    wx = w[0, :] @ np.transpose(X)
    odds_array = np.exp(wx)
    odds = np.median(odds_array)

    w_feat = w[0, index_of_selected_feat]
    odd_ratio = np.exp(w_feat)
    # --------------------------------------------------------------------------
